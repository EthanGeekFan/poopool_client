...
void write_nonce(unsigned int *nonce, __global char *dst) {
   for (int i = 63; i >= 0; i--) {
     unsigned int v = nonce[i / 8];
//     v = SWAP(v);
     v = v >> ((7 - i % 8) * 4);
     v = v & 0xF;
     dst[i] = (v < 0xA) ? '0' + v : 'a' + v - 0xA;
   }
//  for (int i = 7; i >= 0; i--) {
...
// __kernel void hash(__global const unsigned int *in, const int len,
//                    __global unsigned int *out) {
//   int id = get_global_id(0);

//   // __global const unsigned int *payload = in + id * (len / 4);
//   // __global const unsigned int *output = out + id * 8;

//   // sha256(payload, len, output);
//   sha256(in, len, out + id * 8);
// }
